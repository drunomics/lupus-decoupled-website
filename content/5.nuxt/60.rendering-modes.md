...
For a zero-infrastructure alternative, [Lupus CSR](https://www.drupal.org/project/lupus_csr) allows bundling the SPA directly as a Drupal theme — no Node.js server needed. The SPA is served from Drupal and fetches content from the CE API on the same origin.