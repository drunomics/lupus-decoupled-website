...
---
# Component Metadata for Canvas Integration

When components are exposed to [Drupal Canvas](/guide/canvas) via the [component index](/nuxt/component-previews#component-index), Canvas needs to know more than the bare TypeScript types — display labels, descriptions, categories, enum labels, formats, schema references, and so on.

This extra metadata is expressed via JSDoc annotations on the Vue component and its props. [nuxt-component-preview](https://github.com/drunomics/nuxt-component-preview) parses these annotations and lifts them into the generated component index (implemented in [`src/runtime/server/utils/`](https://github.com/drunomics/nuxt-component-preview/tree/1.x/src/runtime/server/utils)).

The pattern is consistent: a label (display name) and an optional description, plus additional tags.
