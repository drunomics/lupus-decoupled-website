...
Pantheon's [Next.js Beta](https://docs.pantheon.io/guides/decoupled/nextjs) hosting can run both **Nuxt** and **Next.js** frontends with Lupus Decoupled Drupal. Despite the name, the platform uses a framework-agnostic Node.js build pipeline (`npm ci` → `npm run build` → `npm prune --production`), so Nuxt works out of the box.
...
::alert{type="warning"}
Do **not** use "Connect existing repository" — it fails silently without any error message. Always let Pantheon create the repo first, then push your code to it.
::
...
- **No server log access**: SSR errors show as generic 500 responses with no way to inspect server-side logs. This makes debugging complex frontend setups difficult.
- **Secrets Manager**: The Terminus CLI doesn't work for Next.js sites. The Dashboard UI for environment variables only works after the first successful deployment.
- **GitHub App permissions**: Pantheon's GitHub App requires broad permissions (read+write to administration, code, deployments, PRs, and workflows).
