...
[Lupus CSR](https://www.drupal.org/project/lupus_csr) is a Drupal base theme that enables serving a pre-built frontend application (e.g. a Nuxt build) directly from Drupal without a separate frontend server. It is the recommended approach for Drupal CMS site templates, demos, ready-made setups, and simple projects that do not require server-side rendering.
